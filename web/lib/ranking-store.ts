...
/** Longest search term we accept — caps the cost of a LIKE scan. */
const SEARCH_MAX_LEN = 60

/** Neutralize LIKE/ILIKE wildcards so a caller can't force a `%%%%` scan. */
function escapeLikeTerm(s: string) {
  return s.replace(/[\\%_]/g, m => '\\' + m)
}

/** Trimmed, length-capped search term (shared by the SQL and in-memory paths). */
export function normalizeRankingSearch(q?: string): string {
  return (q ?? '').trim().slice(0, SEARCH_MAX_LEN)
}

/**
 * WHERE fragment for gamer rows — fully parameterized (Drizzle `sql` template),
 * never string-concatenated, so `disc`/`q` from an unauthenticated caller are
 * bound values rather than SQL text.
 */
function gamerWhere(disc?: string, q?: string) {
  const parts = [sql`u.role = 'gamer'`, sql`u.deleted_at IS NULL`]
  if (disc && disc !== 'all') parts.push(sql`u.primary_disc = ${disc}`)
  const term = normalizeRankingSearch(q)
  if (term) {
    const pat = `%${escapeLikeTerm(term)}%`
    parts.push(sql`(u.name ILIKE ${pat} ESCAPE '\\' OR u.tag ILIKE ${pat} ESCAPE '\\' OR u.city ILIKE ${pat} ESCAPE '\\')`)
  }
...
export async function queryGamerCount(): Promise<number> {
  ...
  const res = await d.execute(sql`SELECT COUNT(*)::int AS n FROM app_users WHERE role = 'gamer' AND deleted_at IS NULL`)
...
export async function queryLeaderboard(opts: LeaderboardQuery = {}): Promise<{ rows: NationalRankRow[]; total: number }> {
  ...
  if (!d) {
    ...
    const term = normalizeRankingSearch(opts.q)
    if (term) {
      const n = term.toLowerCase()
      list = list.filter(r => r.name.toLowerCase().includes(n) || r.tag.toLowerCase().includes(n) || r.city.toLowerCase().includes(n))
    }
    return { rows: list.slice(offset, offset + limit), total: list.length }
  }

  const countRes = await d.execute(sql`
...
export async function queryUserRank(uid: string): Promise<{ rank: number | null; points: number; total: number }> {
  ...
  const res = await d.execute(sql`
    ...
    FROM app_users u
    WHERE u.id = ${uid} AND u.role = 'gamer' AND u.deleted_at IS NULL
  `)
