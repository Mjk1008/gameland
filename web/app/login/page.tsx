...
function LoginInner() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 18px' }}>
      ...
      {/* Google — primary sign-in (shown only when the provider is configured) */}
      {hasGoogle && (
      <div style={{ width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 18 }}>
        <button type="button" onClick={() => signIn('google', { callbackUrl })}
          style={{ all: 'unset', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: '#fff', color: '#1f2937', fontWeight: 700, fontSize: 15, padding: '13px 0', borderRadius: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: '#475569', fontSize: 11 }}>
          <span style={{ flex: 1, height: 1, background: '#1e293b' }}/>یا با موبایل<span style={{ flex: 1, height: 1, background: '#1e293b' }}/>
        </div>
      </div>
      )}
