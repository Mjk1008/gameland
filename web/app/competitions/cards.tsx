...
function FormatChip({ teamSize }: { teamSize?: number }) {
  const isTeam = teamSize === 2
  return (
    <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 6, background: isTeam ? C.accentSoft : 'rgba(246,239,228,.08)', color: isTeam ? C.accent : C.tmut, border: `1px solid ${isTeam ? C.accent + '55' : C.line}` }}>
      {formatModeLabel(teamSize)}
    </span>
  )
}

// Banner (16:9) — coverSrc should come from resolveEventCardCover / resolveCompetitionCardCover.
function Banner({ disc, coverSrc, title, sub, status, discColor, teamSize }: { disc?: string; coverSrc?: string; title: string; sub?: string; status?: string; discColor: string; teamSize?: number }) {
  const img = coverSrc
  return (
    <div style={{ position: 'relative', width: '100%', aspectRatio: '16 / 9', overflow: 'hidden', background: `linear-gradient(135deg, ${discColor}, ${discColor}55)` }}>
      ...
      {status && <div style={{ position: 'absolute', top: 12, insetInlineStart: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}><StatusChip status={status} /><FormatChip teamSize={teamSize} /></div>}
...
export function DisciplineCard({ ev, reg, coverSrc, teamSize }: { ev: Event; reg?: Registration; coverSrc?: string; teamSize?: number }) {
  ...
  const cardTitle = disciplineDisplayName(d?.name ?? ev.disc, teamSize)
  return (
    <Link href={`/competitions/${ev.id}`} style={{ ...cardShell, border: `1px solid ${rs ? rs.c + '66' : 'rgba(246,239,228,.10)'}` }}>
      <Banner disc={ev.disc} coverSrc={coverSrc} title={cardTitle} sub={daysToStart ? `⏳ ${daysToStart} روز تا شروع${ev.date ? ' · ' + ev.date : ''}` : (ev.date || undefined)} status={ev.status} discColor={discColor} teamSize={teamSize} />
      <div style={{ background: bodyBg, padding: '12px 14px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'stretch', background: 'rgba(20,17,13,.45)', border: '1px solid rgba(246,239,228,.07)', borderRadius: 12, overflow: 'hidden' }}>
          ...
          <Cell wide><span style={{ fontSize: 12, fontWeight: 700, color: C.tbody, textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{formatModeLabel(teamSize)}</span><Lab>فرمت</Lab></Cell>
        </div>
      </div>
    </Link>
  )
