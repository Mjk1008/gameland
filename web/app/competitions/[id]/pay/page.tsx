...
import { getEvent, getRegistration, getUserById, receiptCoversPendingPayment, isTeamPartnerReg } from '@/lib/store'
...
export default async function PayPage({ params }: { params: { id: string } }) {
  ...
  return (
    <PayView
      ...
      status={reg.status} hasReceipt={receiptCoversPendingPayment(reg)} disc={c.disc} city={u.city ?? ''}
    />
  )
}
