...
import { DISC, statusColor, avatarBg } from '@/lib/mock-data'
  ...
  return (
    <div className="animate-fade-up" style={{ padding: '14px 16px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 12, color: '#64748b' }}>{events.length} ایونت</span>
      </div>
      ...
      {events.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {events.map((c) => {
            const disc = DISC[c.disc as keyof typeof DISC] ?? { name: c.disc, short: c.disc.toUpperCase().slice(0, 4), color: '#94a3b8' }
