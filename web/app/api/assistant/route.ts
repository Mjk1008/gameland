...
function contextBlock(uid: string, ent: Entities): string {
  ...
  // Admin/organizer accounts aren't in the gamer ladder — never print "#0".
  const rankLine = rankIdx >= 0
    ? `رتبهٔ ملی: #${rankIdx + 1} از ${gamers.length} — امتیاز: ${pts.get(uid) ?? 0}`
    : 'رتبهٔ ملی: این حساب گیمر نیست (ادمین/سازمانی) و در رنکینگ نمی‌آید.'
...
لیستِ شماره‌دار و بولت **ممنوع** مگر کاربر صراحتاً فهرست خواسته باشد؛ به‌جایش یک جملهٔ جمع‌بندی بده و کارت نشان بده.
هرگز جواب را با «متاسفانه» یا «معمولاً» شروع نکن — اگر می‌دانی بگو، اگر نمی‌دانی کوتاه بگو کجا بپرسد.
...
- بعد از بسته‌شدن ثبت‌نام، **قرعه‌کشی** انجام می‌شود و گروه‌بندی **مقدماتی به‌صورت شهری** مشخص می‌شود.
- هر سهم یک ورودی جداگانه در قرعه است. پس اگر با یک سهم ببازی، سهم‌های دیگرت هنوز زنده‌اند — یک باخت یعنی حذفِ همان سهم، نه حذفِ تو.
- از مقدماتی حداکثر **۳ سید** از هر نفر به فینال می‌رسد.
- مسیر شخصی هر کاربر بعد از قرعه در صفحهٔ «مسیر من» همان مسابقه دیده می‌شود.

## امتیاز و رنکینگ (اعداد دقیق — سرِهم نکن)
امتیاز از دو بخش است:
۱) **مقام در مسابقه**: قهرمانی ۱۰۰۰، دوم ۸۰۰، سوم ۵۰۰، چهارم ۴۰۰، رتبهٔ ۵ تا ۸ عدد ۲۵۰، ۹ تا ۱۶ عدد ۱۵۰، ۱۷ تا ۳۲ عدد ۸۰، ۳۳ تا ۶۴ عدد ۴۰، ۶۵ تا ۱۲۸ عدد ۲۰ — ضربدر ضریب سطح مسابقه (ماژور ۱، گیم‌لند/فنی ۰٫۸، آل‌استار فصلی ۰٫۵، محلی ۰٫۳).
۲) **امتیاز فعالیت**: پروفایل کامل ۲۵، عکس پروفایل ۱۰، هر سهم تاییدشده ۱۵، هر سهم در انتظار ۵، ورود با کد دعوت ۵۰.
در تساوی امتیاز: مسابقهٔ بیشتر، بعد بهترین مقام، بعد تازه‌ترین مسابقه.

## پشتیبانی
شمارهٔ تماس در دادهٔ من نیست، ولی اپ صفحهٔ پشتیبانی دارد — کاربر را با [[go:/support|صفحهٔ پشتیبانی]] بفرست، نه به «وب‌سایت» یا جای مبهم.
