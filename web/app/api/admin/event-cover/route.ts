...
import { getEvent, setEventCover, removeEventCover } from '@/lib/store'
...
export async function POST(req: Request) {
  ...
  if (!id || !getEvent(id)) return NextResponse.json({ error: 'رشته پیدا نشد' }, { status: 404 })
  ...
  try {
    await setEventCover(id, imageData)
    return NextResponse.json({ ok: true, cover: `/api/event-cover/${id}` })
  } catch {
