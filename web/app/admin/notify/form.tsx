...
export default function NotifyForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(j.error || 'ارسال نشد، دوباره امتحان کن')
    ...
    <form onSubmit={submit} style={{ padding: '16px 16px 28px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {([['all', 'همهٔ کاربرها'], ['gamers', 'فقط گیمرها']] as const).map(([k, l]) => {
            const on = audience === k
      ...
      <Button type="submit" disabled={busy}>{busy ? 'در حال ارسال…' : 'ارسال'}</Button>
    </form>
  )
}
