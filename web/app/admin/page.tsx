...
export default function AdminHome() {
  ...
  // tile each (was 12 scattered tiles: analytics/behavior/ai, events/
  // competitions/disciplines/new-event, promos/news each had their own tile).
  const tools: { href: string; label: string; sub: string; icon: JSX.Element; badge?: number }[] = [
    { href: '/admin/analytics',   label: 'آنالیتیکس',  sub: 'کسب‌وکار · رفتار کاربران · AI', icon: <IconChart /> },
    { href: '/admin/events',      label: 'مسابقات',    sub: `${events.length} مسابقه · رویدادها · رشته‌ها`, icon: <IconTrophy /> },
    { href: '/admin/content',     label: 'محتوا',      sub: `${slideCount} اسلاید · اخبار`, icon: <IconImage /> },
