...
export type LeftoverOpt = { uid: string; name: string; tag: string; leftover: number }
        ...
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {!target ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {live.map(s => (
                <button key={s.matchId + s.side} type="button" onClick={() => setTarget(s)} style={rowBtn}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                {leftovers.length === 0 && <div style={{ fontSize: 11.5, color: C.tmut }}>کسی نیست</div>}
              </div>
            </div>
          )}
        </div>
