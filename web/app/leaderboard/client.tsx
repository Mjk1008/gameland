...
export default function LeaderboardClient({ initial, meTag, me, cities }: { initial: Row[]; meTag?: string; me?: Row | null; cities?: CityRow[] }) {
  ...
  return (
    <div className="animate-fade-up">
      <div style={{ position: 'sticky', top: 0, zIndex: 5, background: 'rgba(20,17,13,.94)', backdropFilter: 'blur(10px)', padding: '14px 16px 10px', borderBottom: `1px solid ${C.line}` }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 11 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {view === 'players' && <span className="gl-num" style={{ fontSize: 12, color: C.tmut }}>{filtered.length}/{initial.length}</span>}
          </div>
        </div>

        {/* your rank — pinned, always visible */}
        {me && view === 'players' && (
        ...
        {view === 'players' && <div style={{ position: 'relative', marginBottom: 10 }}>
        ...
        {view === 'players' && <div className="gl-scroll" style={{ display: 'flex', gap: 8, overflowX: 'auto', margin: '0 -16px', padding: '0 16px' }}>
          {DISCS.map(d => {
            const on = disc === d.id
            return (
      ...
      {/* city league — same points, aggregated per city */}
      {view === 'cities' && cities && (
